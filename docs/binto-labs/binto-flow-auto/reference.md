...
title: V3 Reference (Binto-Flow-Auto)
author: Claude Code + Human Developer
tags: [reference, v3, commands, architecture, binto-flow, anti-drift, claude-flow]
---

# V3 Reference (Binto-Flow-Auto)
...
```bash
# Initialize swarm (ANTI-DRIFT DEFAULT)
...
```bash
# Initialize hive (ANTI-DRIFT DEFAULT)
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "command": "npx claude-flow@v3alpha hooks pre-edit --file '{}' --auto-assign-agents true --load-context true"
      }]
    }],
    "PostToolUse": [{
      "matcher": "Write|Edit|MultiEdit",
      "hooks": [{
        "command": "npx claude-flow@v3alpha hooks post-edit --file '{}' --format true --update-memory true"
      }]
    }],
    "Stop": [{
      "hooks": [{
        "command": "npx claude-flow@v3alpha hooks session-end --generate-summary true --persist-state true --export-metrics true"
      }]
    }]
  }
}
...
V3 includes background workers that run via the daemon:
...
|------------|-------|-----------|
| All agents | **opus** | Quality matters |
...
**Version**: 2.1 | **Last Updated**: 2026-01-27 | **Track**: B (Progressive)
