...
tags: [hive-mind, queen, swarm, multi-phase, v3, binto-flow, anti-drift, raft]
---
...
**All hive-minds use these defaults:**

```bash
# Pre-flight hook check
npx claude-flow@v3alpha hooks verify || exit 1

# Initialize with anti-drift config + raft consensus
npx claude-flow@v3alpha hive-mind init \
  --topology hierarchical-mesh \
  --consensus raft
```

**Why raft consensus?** Leader maintains authoritative state, preventing drift.

**All agents use opus model.** Quality over cost optimization.
...
# Initialize hive with anti-drift + raft
npx claude-flow@v3alpha hive-mind init \
  --topology hierarchical-mesh \
  --consensus raft
```
...
npx claude-flow@v3alpha memory retrieve --namespace "hive/workers/[ROLE]" --key "assignment"
```
