...
Task(
  ...
  2. READ context from memory:
     npx claude-flow memory retrieve --namespace "swarm/project" --key "plan"
...
Task(
  ...
  2. READ context from swarm memory:
     npx claude-flow memory retrieve --namespace "[namespace]" --key "[key]"
...
while ! npx claude-flow memory retrieve \
  --namespace "swarm/backend" \
...
[Message 2]:
  ...
  Task("Backend Authentication Specialist",
    ...
    2. READ: npx claude-flow memory retrieve --namespace "swarm/project" --key "plan"
    3. TASKS:
  ...
  Task("Frontend Login Specialist",
    ...
    2. WAIT for backend API:
       while ! npx claude-flow memory retrieve --namespace "swarm/backend" --key "auth-api"; do sleep 10; done
    3. TASKS:
  ...
  Task("QA Integration Specialist",
    ...
    2. WAIT for both agents:
       while ! npx claude-flow memory retrieve --namespace "swarm/backend" --key "auth-api"; do sleep 10; done
       while ! npx claude-flow memory retrieve --namespace "swarm/frontend" --key "login-ui-done"; do sleep 10; done
    3. TASKS:
...
Task(
  ...
  2. WAIT: while ! npx claude-flow memory retrieve --namespace "swarm/cart" --key "monitoring-api"; do sleep 10; done
  3. TASKS:
