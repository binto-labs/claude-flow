...
while true; do
  npx claude-flow@alpha memory retrieve --namespace "hive/worker-*" --key "report"
...
npx claude-flow@alpha memory retrieve --namespace "hive/[PROJECT]" --key "plan"
npx claude-flow@alpha memory retrieve --namespace "hive/queen" --key "[YOUR-ASSIGNMENT]"
...
# Verify completion
npx claude-flow@alpha memory retrieve --namespace "hive/queen" --key "mission-complete"
```
